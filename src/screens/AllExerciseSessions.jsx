...
import yoga from '../assetes/exercise.png'

const AllExerciseSessions = () => {
  const { userData, exerciseSessions ,navigate} = useContext(AppContext);

  if (!exerciseSessions || exerciseSessions.length === 0) {
    return (
      <div>
        <UserNavbar />
        <div className="text-center mt-5 text-muted">No yoga sessions available.</div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="container mt-5 py-4">
        <h2 className="text-center mb-5 fw-bold text-success">Explore All Yoga Sessions</h2>
        <div className="row">
          {exerciseSessions.map((session) => (
     <div className="col-md-6 col-lg-4 mb-4" key={session._id} >
  <div className="card h-100 shadow-sm border rounded-4 bg-light "  style={{ boxShadow: "0 4px 20px rgba(0, 0, 0, 0.08)" }}>
    <div className="card-body p-4 d-flex flex-column text-start justify-content-start">
      ...
      <button
                    ...
                    onClick={() => navigate(`/exercise-session-details/${session._id}`)}
                  >
                    View
                  </button>
...
export default AllExerciseSessions;
